...
public:
	...
	void outline(std::ofstream& writer, const std::string& savename, bitmap_image image);

private:
	std::array<const char, 8> map_chars = { '@', '&', '#', 'B', 'x', '+', ':', '`' };
	std::array<const char, 8> outline_chars = { ' ', '/', '\\', '-', '_', '|' };
	...
	char choose_map_char(unsigned int r, unsigned int g, unsigned int b);
	void pixelate(bitmap_image& image);
};
